...
cmd = sprintf('moteSelfTriggered %u 0 %u %u %u %u', ...
    ...
    WTSelf.nSensors, ...
    WTSelf.channel, ...
    WTSelf.panID, .../home/kthwsn/workspace/apps.water-tank/SelfTriggeredJoseLast/matlab/serial_output
    WTSelf.offsetWT);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% experimentLOWP = 0 => periodic slow
% experimentLOWP = 1 => periodic fast
% experimentLOWP = 2 => burst

experimentLOWP=2;
%WTSelf.tBI=0.644; % remove when running in reality!!!!!
stepBURST=1*WTSelf.tBI;
stepSLOW=5*WTSelf.tBI;
BURSTduration=25*WTSelf.tBI;
Trise=100*WTSelf.tBI; % in seconds
Tdisturbance=200*WTSelf.tBI-10*WTSelf.tBI;
Tsim=450*WTSelf.tBI;
TimeToStart=0;
randomtostart=0.01;

switch experimentLOWP
    case 0 % periodic slow
        
        for kkk=1:WTSelf.nSensors
        WTSelf.requesttimes{kkk} = [(TimeToStart+kkk*randomtostart):stepSLOW:Tsim];
        
        end
       
    case 1 % periodic fast
        
        for kkk=1:WTSelf.nSensors
        WTSelf.requesttimes{kkk} = [(TimeToStart+kkk*randomtostart):stepBURST:Tsim];
        end
        
    case 2 % bursty
        
        for kkk=1:WTSelf.nSensors
        burst1=[(TimeToStart+kkk*randomtostart):stepBURST:BURSTduration BURSTduration+WTSelf.tBI:stepSLOW:Trise];
        burst2=[Trise:stepBURST:Trise+BURSTduration Trise+BURSTduration:stepSLOW:Tdisturbance];
        burst3=[Tdisturbance:stepBURST:Tdisturbance+BURSTduration Tdisturbance+BURSTduration:stepSLOW:Tsim];
        
        WTSelf.requesttimes{kkk} = [burst1 burst2 burst3];
        
        end
        %
end
%stem(requesttimes{1}, ones(1,length(requesttimes{1})));
%NUMPKTSGENERATED=length(requesttimes{1})

WTSelf.Nsamplesrequested = zeros(WTSelf.nSensors,1);
WTSelf.Nsamplestriggered = zeros(WTSelf.nSensors,1);
% triggtimes is the matrix that keeps track of the times that the low
% priority nodes actually took place
for kk=1:WTSelf.nSensors
    WTSelf.triggtimes{kk} = 0;    
    % pCurrentPkt is the pointer to the current pkt to be transmitted
    WTSelf.pCurrentPkt(1,kk)=1;
end
