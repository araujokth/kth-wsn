...
static void mdlInitializeSizes(SimStruct *S)
{
 ...
 char host[] = "localhost";
    ...
    ssSetInputPortWidth(S, 0, WT_NUMBER);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    uint32_T *y = ssGetOutputPortSignal(S,0);
    uint16_T *uOut = ssGetOutputPortSignal(S,1);

    const uint16_T *u = (const uint16_T *) ssGetInputPortSignal(S,0);
    int_T width = ssGetInputPortWidth(S,0);

    /* Packet format used by TinyOS 2.x */
    packet[0] = 0x00;
    packet[1] = 0xff;
    packet[2] = 0xff;
    packet[3] = 0x00;
    packet[4] = 0x00;
    packet[5] = (uint8_T) width*2; /* payload length */
    packet[6] = 0x00;
    packet[7] = 11; /*packet id */

    i=HEADER_LEN;
    payload = packet + i;
    for (j = 0; j < width ; j++){       
        *payload++  = (uint8_T) (*(u+j) >> 8) & 0xFF;
        *payload++ = (uint8_T) (*(u+j)) & 0xFF;
    }         
    
    /* Send packet to the SerialForwarder server using functions linked in sfsource.o */
    write_sf_packet(fd, (void *) packet,  packet[5] + HEADER_LEN);
