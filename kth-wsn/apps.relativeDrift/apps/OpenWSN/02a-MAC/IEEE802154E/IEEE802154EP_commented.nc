...
implementation {
	...
	float a_drift;
	float a_currentDrift;
	float b_offset;
	float b_currentOffset;
	int Tr1_resynchTime;
	int Tr2_resynchTime;
	float tmp1;
	float tmp2;
	...
	async event void SlotAlarm.fired() {
		...
		atomic {
			if (change) {
				...
				numSlotsInSynchStatus=0;
				numPrepareReceiveFailed=0;
				numADVtoResynchronize=0;
				counterNumADVtoResynchronize=0;
				drift = FALSE;
			}
		}
		...
		switch (temp_state) {
			...
			if (call IDManager.isMyAddress(&(received_ieee154_header.dest)) && received_ieee154_header.ackRequested) {
				...
				if (received_ieee154_header.frameType==IEEE154_TYPE_CMD &&
					...
					atomic {
						if (numADV==1) {
							timestampX1 = ((IEEE802154E_ADV_t*)(msg->payload))->timestampInformation;
							timestampY1 = call SlotAlarm.getNow();
							printf("%lu %lu\n", timestampX1, timestampY1);
							printfflush();
						} else if (numADV==240) {
							timestampX2 = ((IEEE802154E_ADV_t*)(msg->payload))->timestampInformation;
							timestampY2 = call SlotAlarm.getNow();
							a_drift = ((float)(timestampY2-timestampY1))/((float)(timestampX2-timestampX1));
							b_offset = ((float)timestampY2) - a_drift*((float)timestampX2);
							printf("%lu %lu ", timestampX2, timestampY2);
							printfFloat(a_drift);
							printfFloat(b_offset);
							printf("\n");
							printfflush();
							...
							timestampX2 = ((IEEE802154E_ADV_t*)(msg->payload))->timestampInformation;
							timestampY2 = call SlotAlarm.getNow();
							a_currentDrift = ((float)(timestampY2-timestampY1))/((float)(timestampX2-timestampX1));
							b_currentOffset = ((float)timestampY2) - a_currentDrift*((float)timestampX2);
							tmp1 = (GAMMA*((float)(a_drift - a_currentDrift)));
							tmp2 = (GAMMA*((float)(b_offset - b_currentOffset)));
							Tr1_resynchTime = (int)((((float)D_THRESHOLD)-(tmp2))/(tmp1));
							Tr2_resynchTime = (int)((-((float)D_THRESHOLD)-(tmp2))/(tmp1));
							...
							numADVtoResynchronize = (Tr_resynchTime)/((uint32_t)(SLOT_TIME));
							counterNumADVtoResynchronize = numADV + ((uint32_t)numADVtoResynchronize);
							printf("%lu %lu ", timestampX2, timestampY2);
							printfFloat(a_currentDrift);
							printfFloat(b_currentOffset);
							printf("%lu",Tr_resynchTime);
							printf("\n");
							timestampX1 = timestampX2;
							timestampY1 = timestampY2;
							a_drift = a_currentDrift;
							b_offset = b_currentOffset;
						}
					}
				}
