close all
SHOW_TABLES_ENABLED = true;
INF_VALUE = 10*60*1000;
MIN_CONV = 1000*60; % 1 ms = 1/ (1000*60) min
CLEAR_PAGE = 15;

folder = '/home/david/Desktop/projects/criticalTime/resultsTables/criticalTime_10_tmote_noHop_indoor_23_08/';
d = dir(folder);
d = d(3:end);
files = [];
cnt = 0;
...
tmp2Str={};
tmpStr={};
tmp = reshape (meanLinks,TCAapp.motelist.nMotes-1,TCAapp.motelist.nMotes);
% tmpStr = reshape (linksStr(1,1:end),TCAapp.motelist.nMotes-1,TCAapp.motelist.nMotes);
for i=1:(TCAapp.motelist.nMotes-1)
  tmp1=[tmp1; tmp(i:(TCAapp.motelist.nMotes-1),i)];
  tmp2=[tmp2 tmp(i,1+i:TCAapp.motelist.nMotes)];
%   tmp1Str=[tmp1Str; tmpStr(i:(TCAapp.motelist.nMotes-1),i)];
%   tmp2Str=[tmp2Str tmpStr(i,1+i:TCAapp.motelist.nMotes)];
  
end
for i=1:(TCAapp.motelist.nMotes-1)
    for j=(i+1):(TCAapp.motelist.nMotes)
        tmp1Str = [tmp1Str sprintf('T%d - R%d', i,j)];
        tmp2Str = [tmp2Str sprintf('T%d - R%d', j,i)];
    end
end

% linksStr{i, n} = sprintf('T%d - R%d', i, );
tmp1 = transpose(tmp1);
tmp2 = flipdim(tmp2,2);
tmp2Str = flipdim(tmp2Str,2);
tmp = [tmp2 tmp1];
tmpStr = [tmp2Str tmp1Str];

figure;
plot (tmp,'--x');
xlim([1 (TCAapp.motelist.nMotes)*(TCAapp.motelist.nMotes-1)]);
set(gca,'XTick',1:1:(TCAapp.motelist.nMotes)*(TCAapp.motelist.nMotes-1))
set(gca,'XTickLabel',tmpStr);
ylabel('Critical Time [ms]','fontweight','b');
xlabel('Transmitter # - Receiver #','fontweight','b');
...
xlabel('Critical Time [ms]','fontweight','b');
