...
static SFc6_sensor_blockInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c6_sensor_block(void);
static void initialize_params_c6_sensor_block(void);
static void enable_c6_sensor_block(void);
static void disable_c6_sensor_block(void);
static void c6_update_debugger_state_c6_sensor_block(void);
static const mxArray *get_sim_state_c6_sensor_block(void);
static void set_sim_state_c6_sensor_block(const mxArray *c6_st);
static void finalize_c6_sensor_block(void);
static void sf_c6_sensor_block(void);
...
static void initialize_c6_sensor_block(void)
{
  ...
  chartInstance.c6_is_active_c6_sensor_block = 0U;
}
...
static const mxArray *get_sim_state_c6_sensor_block(void)
{
  ...
  c6_f_u = chartInstance.c6_is_active_c6_sensor_block;
  c6_g_y = NULL;
...
static void set_sim_state_c6_sensor_block(const mxArray *c6_st)
{
  ...
  real_T c6_d3;
  real_T c6_e_y;
  const mxArray *c6_b_is_active_c6_sensor_block;
  uint8_T c6_u0;
  uint8_T c6_f_y;
  real_T *c6_d_delay;
  real_T *c6_errors;
  ...
  *c6_reliability = c6_c_y;
  ...
  c6_b_is_active_c6_sensor_block = sf_mex_dup(sf_mex_getcell(c6_u, 5));
  sf_mex_import("is_active_c6_sensor_block", sf_mex_dup
                (c6_b_is_active_c6_sensor_block), &c6_u0, 1, 3, 0U, 0, 0U, 0);
  c6_f_y = c6_u0;
  sf_mex_destroy(&c6_b_is_active_c6_sensor_block);
  chartInstance.c6_is_active_c6_sensor_block = c6_f_y;
  sf_mex_destroy(&c6_u);
  c6_update_debugger_state_c6_sensor_block();
...
const mxArray *sf_c6_sensor_block_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c6_sensor_block_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c6_sensor_block_get_autoinheritance_info(void)
{
...
static mxArray *sf_get_sim_state_info_c6_sensor_block(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  char *infoEncStr[] = {
    "100 S1x6'type','srcId','name','auxInfo'{{M[1],M[32],T\"delay\",},{M[1],M[10],T\"errors\",},{M[1],M[11],T\"reliability\",},{M[4],M[0],T\"checkLinks\",S'l','i','p'{{M1x2[269 279],M[0],}}},{M[4],M[0],T\"idx\",S'l','i','p'{{M1x2[280 283],M[0],}}},{M[8],M[0],T\"is_active_c6_sensor_block\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 6, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c6_sensor_block_get_check_sum(&mxChecksum);
...
static void sf_opaque_initialize_c6_sensor_block(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c6_sensor_block();
  initialize_c6_sensor_block();
}

static void sf_opaque_enable_c6_sensor_block(void *chartInstanceVar)
{
  enable_c6_sensor_block();
}

static void sf_opaque_disable_c6_sensor_block(void *chartInstanceVar)
{
  disable_c6_sensor_block();
}

static void sf_opaque_gateway_c6_sensor_block(void *chartInstanceVar)
{
  sf_c6_sensor_block();
}

static mxArray* sf_opaque_get_sim_state_c6_sensor_block(void *chartInstanceVar)
{
  mxArray *st = (mxArray *) get_sim_state_c6_sensor_block();
  return st;
}

static void sf_opaque_set_sim_state_c6_sensor_block(void *chartInstanceVar,
  const mxArray *st)
{
  set_sim_state_c6_sensor_block(sf_mex_dup(st));
}

static void sf_opaque_terminate_c6_sensor_block(void *chartInstanceVar)
{
  ...
  finalize_c6_sensor_block();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c6_sensor_block(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c6_sensor_block(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("sensor_block","sensor_block",6);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop("sensor_block","sensor_block",6,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("sensor_block",
      "sensor_block",6,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"sensor_block","sensor_block",6,10);
      sf_mark_chart_reusable_outputs(S,"sensor_block","sensor_block",6,3);
    }

    sf_set_rtw_dwork_info(S,"sensor_block","sensor_block",6);
...
static void mdlRTW_c6_sensor_block(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "sensor_block", "sensor_block",6);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c6_sensor_block(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway = sf_opaque_gateway_c6_sensor_block;
  chartInstance.chartInfo.initializeChart = sf_opaque_initialize_c6_sensor_block;
  chartInstance.chartInfo.terminateChart = sf_opaque_terminate_c6_sensor_block;
  chartInstance.chartInfo.enableChart = sf_opaque_enable_c6_sensor_block;
  chartInstance.chartInfo.disableChart = sf_opaque_disable_c6_sensor_block;
  chartInstance.chartInfo.getSimState = sf_opaque_get_sim_state_c6_sensor_block;
  chartInstance.chartInfo.setSimState = sf_opaque_set_sim_state_c6_sensor_block;
  chartInstance.chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c6_sensor_block;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c6_sensor_block;
  chartInstance.chartInfo.mdlStart = mdlStart_c6_sensor_block;
  chartInstance.chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c6_sensor_block;
...
void c6_sensor_block_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c6_sensor_block(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c6_sensor_block(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c6_sensor_block(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c6_sensor_block_method_dispatcher.\n"
