...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_sensor_block_pulse_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_sensor_block_pulse_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_sensor_block_pulse_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_sensor_block_pulse_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_sensor_block_pulse_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "sensor_block_pulse") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_sensor_block_pulse_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_sensor_block_pulse_method_dispatcher(SimStruct *S,
...
{
  if (!strcmp(machineName,"sensor_block_pulse")) {
    return(sf_sensor_block_pulse_method_dispatcher(simstructPtr,chartFileNumber,
            method,data));
  }
...
extern void sensor_block_pulse_terminator(void);
...
extern void sensor_block_pulse_initializer(void);
extern void sensor_block_pulse_register_exported_symbols(SimStruct* S);
extern void sensor_block_pulse_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  sensor_block_pulse_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      sensor_block_pulse_debug_initialize();
    }

    sensor_block_pulse_initializer();
  }
