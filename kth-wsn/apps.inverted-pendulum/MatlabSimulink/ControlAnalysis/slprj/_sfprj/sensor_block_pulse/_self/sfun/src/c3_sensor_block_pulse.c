...
static SFc3_sensor_block_pulseInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c3_sensor_block_pulse(void);
static void initialize_params_c3_sensor_block_pulse(void);
static void enable_c3_sensor_block_pulse(void);
static void disable_c3_sensor_block_pulse(void);
static void c3_update_debugger_state_c3_sensor_block_pulse(void);
static const mxArray *get_sim_state_c3_sensor_block_pulse(void);
static void set_sim_state_c3_sensor_block_pulse(const mxArray *c3_st);
static void finalize_c3_sensor_block_pulse(void);
static void sf_c3_sensor_block_pulse(void);
static void c3_c3_sensor_block_pulse(void);
...
static void initialize_c3_sensor_block_pulse(void)
{
  ...
  chartInstance.c3_is_active_c3_sensor_block_pulse = 0U;
}
...
static const mxArray *get_sim_state_c3_sensor_block_pulse(void)
{
  ...
  c3_p_u = chartInstance.c3_is_active_c3_sensor_block_pulse;
  c3_q_y = NULL;
...
static void set_sim_state_c3_sensor_block_pulse(const mxArray *c3_st)
{
  ...
  real_T c3_d11;
  real_T c3_o_y;
  const mxArray *c3_b_is_active_c3_sensor_block_pulse;
  uint8_T c3_u0;
  uint8_T c3_p_y;
  ...
  *c3_xc_value = c3_i_y;
  ...
  c3_b_is_active_c3_sensor_block_pulse = sf_mex_dup(sf_mex_getcell(c3_u, 15));
  sf_mex_import("is_active_c3_sensor_block_pulse", sf_mex_dup
                (c3_b_is_active_c3_sensor_block_pulse), &c3_u0, 1, 3, 0U, 0, 0U,
                0);
  c3_p_y = c3_u0;
  sf_mex_destroy(&c3_b_is_active_c3_sensor_block_pulse);
  chartInstance.c3_is_active_c3_sensor_block_pulse = c3_p_y;
  sf_mex_destroy(&c3_u);
  c3_update_debugger_state_c3_sensor_block_pulse();
...
static void c3_c3_sensor_block_pulse(void)
{
  ...
  _SFD_EML_CALL(0,35);
  if (CV_EML_IF(0, 1, (real_T)c3_src_addr <= (c3_NUMBER_SENSORS +
        c3_NUMBER_LINKS) - 1.0)) {
    /*  */
    /*  SENSOR ---- > COORDINATOR */
    /*  */
  ...
  c3_b_x = c3_x;
  c3_c_x = c3_b_x;
  c3_theta_value = c3_c_x / 1024.0;

  /*  * 2 * 3.14159265; */
  _SFD_EML_CALL(0,67);
  c3_c_a = chartInstance.c3_theta_delay_p;
  c3_theta_delay = c3_c_a * 1.5300814003304975E-02;
  _SFD_EML_CALL(0,69);
  c3_actuator_value = chartInstance.c3_actuator_value_p;
  _SFD_EML_CALL(0,70);
  c3_d_a = chartInstance.c3_actuator_delay_p;
  c3_actuator_delay = c3_d_a * 1.5300814003304975E-02;
  _SFD_EML_CALL(0,-70);
...
const mxArray *sf_c3_sensor_block_pulse_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c3_sensor_block_pulse_get_check_sum(mxArray *plhs[])
{
  ((real_T *)mxGetPr((plhs[0])))[0] = (real_T)(2875032173U);
  ((real_T *)mxGetPr((plhs[0])))[1] = (real_T)(3958268595U);
  ((real_T *)mxGetPr((plhs[0])))[2] = (real_T)(1919777708U);
  ((real_T *)mxGetPr((plhs[0])))[3] = (real_T)(102188935U);
}

mxArray *sf_c3_sensor_block_pulse_get_autoinheritance_info(void)
{
...

  {
    ...
    pr[0] = (double)(233622285U);
    pr[1] = (double)(92658412U);
    pr[2] = (double)(3245587302U);
    pr[3] = (double)(2950244648U);
...
static mxArray *sf_get_sim_state_info_c3_sensor_block_pulse(void)
{
  ...
  if (!sim_mode_is_rtw_gen(S)) {
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        _SFD_CV_INIT_EML_FCN(0,0,"eML_blk_kernel",0,-1,1867);
        _SFD_CV_INIT_EML_IF(0,0,501,523,-1,729);
        _SFD_CV_INIT_EML_IF(0,1,800,852,-1,1608);
        _SFD_CV_INIT_EML_IF(0,2,981,1009,1157,1600);
        _SFD_CV_INIT_EML_IF(0,3,1157,1193,1349,1600);
        _SFD_CV_INIT_EML_IF(0,4,1349,1387,-1,1387);
...
static void sf_opaque_initialize_c3_sensor_block_pulse(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c3_sensor_block_pulse();
  initialize_c3_sensor_block_pulse();
}

static void sf_opaque_enable_c3_sensor_block_pulse(void *chartInstanceVar)
{
  enable_c3_sensor_block_pulse();
}

static void sf_opaque_disable_c3_sensor_block_pulse(void *chartInstanceVar)
{
  disable_c3_sensor_block_pulse();
}

static void sf_opaque_gateway_c3_sensor_block_pulse(void *chartInstanceVar)
{
  sf_c3_sensor_block_pulse();
}

static mxArray* sf_opaque_get_sim_state_c3_sensor_block_pulse(void
  *chartInstanceVar)
{
  mxArray *st = (mxArray *) get_sim_state_c3_sensor_block_pulse();
  return st;
}

static void sf_opaque_set_sim_state_c3_sensor_block_pulse(void *chartInstanceVar,
  const mxArray *st)
{
  set_sim_state_c3_sensor_block_pulse(sf_mex_dup(st));
}

static void sf_opaque_terminate_c3_sensor_block_pulse(void *chartInstanceVar)
{
  ...
  finalize_c3_sensor_block_pulse();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c3_sensor_block_pulse(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c3_sensor_block_pulse(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("sensor_block_pulse","sensor_block_pulse",3);
    ...
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("sensor_block_pulse",
      "sensor_block_pulse",3,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 2, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"sensor_block_pulse",
        "sensor_block_pulse",3,3);
      sf_mark_chart_reusable_outputs(S,"sensor_block_pulse","sensor_block_pulse",
        3,9);
    }

    sf_set_rtw_dwork_info(S,"sensor_block_pulse","sensor_block_pulse",3);
    ssSetHasSubFunctions(S,!(chartIsInlinable));
    ssSetOptions(S,ssGetOptions(S)|SS_OPTION_WORKS_WITH_CODE_REUSE);
  }

  ssSetChecksum0(S,(1896025974U));
  ssSetChecksum1(S,(3644126555U));
  ssSetChecksum2(S,(799901486U));
  ssSetChecksum3(S,(1356858978U));
  ssSetmdlDerivatives(S, NULL);
  ssSetExplicitFCSSCtrl(S,1);
}

static void mdlRTW_c3_sensor_block_pulse(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "sensor_block_pulse", "sensor_block_pulse",3);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c3_sensor_block_pulse(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c3_sensor_block_pulse;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c3_sensor_block_pulse;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c3_sensor_block_pulse;
  chartInstance.chartInfo.enableChart = sf_opaque_enable_c3_sensor_block_pulse;
  chartInstance.chartInfo.disableChart = sf_opaque_disable_c3_sensor_block_pulse;
  chartInstance.chartInfo.getSimState =
    sf_opaque_get_sim_state_c3_sensor_block_pulse;
  chartInstance.chartInfo.setSimState =
    sf_opaque_set_sim_state_c3_sensor_block_pulse;
  chartInstance.chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c3_sensor_block_pulse;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c3_sensor_block_pulse;
  chartInstance.chartInfo.mdlStart = mdlStart_c3_sensor_block_pulse;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c3_sensor_block_pulse;
...
void c3_sensor_block_pulse_method_dispatcher(SimStruct *S, int_T method, void
  *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c3_sensor_block_pulse(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c3_sensor_block_pulse(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c3_sensor_block_pulse(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c3_sensor_block_pulse_method_dispatcher.\n"
