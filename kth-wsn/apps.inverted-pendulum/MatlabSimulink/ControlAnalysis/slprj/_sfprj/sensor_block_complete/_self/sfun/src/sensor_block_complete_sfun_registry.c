...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_sensor_block_complete_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_sensor_block_complete_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_sensor_block_complete_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_sensor_block_complete_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_sensor_block_complete_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "sensor_block_complete") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_sensor_block_complete_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_sensor_block_complete_method_dispatcher(SimStruct *S,
...
{
  if (!strcmp(machineName,"sensor_block_complete")) {
    return(sf_sensor_block_complete_method_dispatcher(simstructPtr,
...
extern void sensor_block_complete_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    sensor_block_complete_terminator();
    sf_debug_terminate();
  }
...
extern void sensor_block_complete_initializer(void);
extern void sensor_block_complete_register_exported_symbols(SimStruct* S);
extern void sensor_block_complete_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  sensor_block_complete_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      sensor_block_complete_debug_initialize();
    }

    sensor_block_complete_initializer();
  }
