...
#include "c1_sensor_block_complete.h"
...
static SFc1_sensor_block_completeInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c1_sensor_block_complete(void);
static void initialize_params_c1_sensor_block_complete(void);
static void enable_c1_sensor_block_complete(void);
static void disable_c1_sensor_block_complete(void);
static void c1_update_debugger_state_c1_sensor_block_complete(void);
static const mxArray *get_sim_state_c1_sensor_block_complete(void);
static void set_sim_state_c1_sensor_block_complete(const mxArray *c1_st);
static void finalize_c1_sensor_block_complete(void);
static void sf_c1_sensor_block_complete(void);
...
static void initialize_c1_sensor_block_complete(void)
{
  ...
  chartInstance.c1_is_active_c1_sensor_block_complete = 0U;
}
...
static const mxArray *get_sim_state_c1_sensor_block_complete(void)
{
  ...
  c1_h_u = chartInstance.c1_is_active_c1_sensor_block_complete;
  c1_i_y = NULL;
...
static void set_sim_state_c1_sensor_block_complete(const mxArray *c1_st)
{
  ...
  uint16_T c1_u1;
  uint16_T c1_g_y;
  const mxArray *c1_b_is_active_c1_sensor_block_complete;
  uint8_T c1_u2;
  uint8_T c1_h_y;
  real_T *c1_c_errors;
  real_T *c1_reliability;
  ...
  *c1_reliability = c1_b_y;
  ...
  c1_b_is_active_c1_sensor_block_complete = sf_mex_dup(sf_mex_getcell(c1_u, 7));
  sf_mex_import("is_active_c1_sensor_block_complete", sf_mex_dup
                (c1_b_is_active_c1_sensor_block_complete), &c1_u2, 1, 3, 0U, 0,
                0U, 0);
  c1_h_y = c1_u2;
  sf_mex_destroy(&c1_b_is_active_c1_sensor_block_complete);
  chartInstance.c1_is_active_c1_sensor_block_complete = c1_h_y;
  sf_mex_destroy(&c1_u);
  c1_update_debugger_state_c1_sensor_block_complete();
...
const mxArray *sf_c1_sensor_block_complete_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c1_sensor_block_complete_get_check_sum(mxArray *plhs[])
{
  ((real_T *)mxGetPr((plhs[0])))[0] = (real_T)(3290703405U);
  ((real_T *)mxGetPr((plhs[0])))[1] = (real_T)(3775240537U);
  ((real_T *)mxGetPr((plhs[0])))[2] = (real_T)(631565771U);
  ((real_T *)mxGetPr((plhs[0])))[3] = (real_T)(370608728U);
}

mxArray *sf_c1_sensor_block_complete_get_autoinheritance_info(void)
{
...

  {
    ...
    pr[0] = (double)(1564807338U);
    pr[1] = (double)(3238963930U);
    pr[2] = (double)(4001787735U);
    pr[3] = (double)(3017634162U);
  ...
  if (!sim_mode_is_rtw_gen(S)) {
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        _SFD_CV_INIT_EML_FCN(0,0,"eML_blk_kernel",0,-1,1873);
        ...
        _SFD_CV_INIT_EML_IF(0,5,1756,1782,1804,1873);
...
static void sf_opaque_initialize_c1_sensor_block_complete(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c1_sensor_block_complete();
  initialize_c1_sensor_block_complete();
}

static void sf_opaque_enable_c1_sensor_block_complete(void *chartInstanceVar)
{
  enable_c1_sensor_block_complete();
}

static void sf_opaque_disable_c1_sensor_block_complete(void *chartInstanceVar)
{
  disable_c1_sensor_block_complete();
}

static void sf_opaque_gateway_c1_sensor_block_complete(void *chartInstanceVar)
{
  sf_c1_sensor_block_complete();
}

static mxArray* sf_opaque_get_sim_state_c1_sensor_block_complete(void
  *chartInstanceVar)
{
  mxArray *st = (mxArray *) get_sim_state_c1_sensor_block_complete();
  return st;
}

static void sf_opaque_set_sim_state_c1_sensor_block_complete(void
  *chartInstanceVar, const mxArray *st)
{
  set_sim_state_c1_sensor_block_complete(sf_mex_dup(st));
}

static void sf_opaque_terminate_c1_sensor_block_complete(void *chartInstanceVar)
{
  ...
  finalize_c1_sensor_block_complete();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c1_sensor_block_complete(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c1_sensor_block_complete(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("sensor_block_complete",
      "sensor_block_complete",1);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop("sensor_block_complete",
                "sensor_block_complete",1,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("sensor_block_complete",
      "sensor_block_complete",1,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 2, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"sensor_block_complete",
        "sensor_block_complete",1,3);
      sf_mark_chart_reusable_outputs(S,"sensor_block_complete",
        "sensor_block_complete",1,2);
    }

    sf_set_rtw_dwork_info(S,"sensor_block_complete","sensor_block_complete",1);
    ssSetHasSubFunctions(S,!(chartIsInlinable));
    ssSetOptions(S,ssGetOptions(S)|SS_OPTION_WORKS_WITH_CODE_REUSE);
  }

  ssSetChecksum0(S,(3975985603U));
  ssSetChecksum1(S,(3440210393U));
  ssSetChecksum2(S,(2966008448U));
  ssSetChecksum3(S,(2482517963U));
  ssSetmdlDerivatives(S, NULL);
  ssSetExplicitFCSSCtrl(S,1);
}

static void mdlRTW_c1_sensor_block_complete(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c1_sensor_block_complete(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c1_sensor_block_complete;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c1_sensor_block_complete;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c1_sensor_block_complete;
  chartInstance.chartInfo.enableChart =
    sf_opaque_enable_c1_sensor_block_complete;
  chartInstance.chartInfo.disableChart =
    sf_opaque_disable_c1_sensor_block_complete;
  chartInstance.chartInfo.getSimState =
    sf_opaque_get_sim_state_c1_sensor_block_complete;
  chartInstance.chartInfo.setSimState =
    sf_opaque_set_sim_state_c1_sensor_block_complete;
  chartInstance.chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c1_sensor_block_complete;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c1_sensor_block_complete;
  chartInstance.chartInfo.mdlStart = mdlStart_c1_sensor_block_complete;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c1_sensor_block_complete;
...
void c1_sensor_block_complete_method_dispatcher(SimStruct *S, int_T method, void
  *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c1_sensor_block_complete(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c1_sensor_block_complete(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c1_sensor_block_complete(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c1_sensor_block_complete_method_dispatcher.\n"
