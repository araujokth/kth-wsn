...
#include "c6_sensor_block_together.h"
...
static SFc6_sensor_block_togetherInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c6_sensor_block_together(void);
static void initialize_params_c6_sensor_block_together(void);
static void enable_c6_sensor_block_together(void);
static void disable_c6_sensor_block_together(void);
static void c6_update_debugger_state_c6_sensor_block_together(void);
static const mxArray *get_sim_state_c6_sensor_block_together(void);
static void set_sim_state_c6_sensor_block_together(const mxArray *c6_st);
static void finalize_c6_sensor_block_together(void);
static void sf_c6_sensor_block_together(void);
...
static void initialize_c6_sensor_block_together(void)
{
  ...
  chartInstance.c6_is_active_c6_sensor_block_together = 0U;
}
...
static const mxArray *get_sim_state_c6_sensor_block_together(void)
{
  ...
  c6_f_u = chartInstance.c6_is_active_c6_sensor_block_together;
  c6_g_y = NULL;
...
static void set_sim_state_c6_sensor_block_together(const mxArray *c6_st)
{
  ...
  real_T c6_d3;
  real_T c6_e_y;
  const mxArray *c6_b_is_active_c6_sensor_block_together;
  uint8_T c6_u0;
  uint8_T c6_f_y;
  real_T *c6_d_delay;
  real_T *c6_errors;
  ...
  *c6_reliability = c6_c_y;
  ...
  c6_b_is_active_c6_sensor_block_together = sf_mex_dup(sf_mex_getcell(c6_u, 5));
  sf_mex_import("is_active_c6_sensor_block_together", sf_mex_dup
                (c6_b_is_active_c6_sensor_block_together), &c6_u0, 1, 3, 0U, 0,
                0U, 0);
  c6_f_y = c6_u0;
  sf_mex_destroy(&c6_b_is_active_c6_sensor_block_together);
  chartInstance.c6_is_active_c6_sensor_block_together = c6_f_y;
  sf_mex_destroy(&c6_u);
  c6_update_debugger_state_c6_sensor_block_together();
...
static void sf_c6_sensor_block_together(void)
{
  ...
  sf_debug_symbol_scope_add("samples", &c6_samples, c6_sf_marshall);
...
const mxArray *sf_c6_sensor_block_together_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c6_sensor_block_together_get_check_sum(mxArray *plhs[])
{
  ((real_T *)mxGetPr((plhs[0])))[0] = (real_T)(2975622442U);
  ((real_T *)mxGetPr((plhs[0])))[1] = (real_T)(2237584900U);
  ((real_T *)mxGetPr((plhs[0])))[2] = (real_T)(803743054U);
  ((real_T *)mxGetPr((plhs[0])))[3] = (real_T)(183766456U);
}

mxArray *sf_c6_sensor_block_together_get_autoinheritance_info(void)
{
...

  {
    ...
    pr[0] = (double)(2463952937U);
    pr[1] = (double)(2374046001U);
    pr[2] = (double)(3029942004U);
    pr[3] = (double)(3077268631U);
...
static mxArray *sf_get_sim_state_info_c6_sensor_block_together(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  char *infoEncStr[] = {
    "100 S1x6'type','srcId','name','auxInfo'{{M[1],M[32],T\"delay\",},{M[1],M[10],T\"errors\",},{M[1],M[11],T\"reliability\",},{M[4],M[0],T\"checkLinks\",S'l','i','p'{{M1x2[269 279],M[0],}}},{M[4],M[0],T\"idx\",S'l','i','p'{{M1x2[280 283],M[0],}}},{M[8],M[0],T\"is_active_c6_sensor_block_together\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 6, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c6_sensor_block_together_get_check_sum(&mxChecksum);
  ...
  if (!sim_mode_is_rtw_gen(S)) {
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        _SFD_CV_INIT_EML_IF(0,0,318,340,-1,417);
        _SFD_CV_INIT_EML_IF(0,1,418,442,-1,560);
        _SFD_CV_INIT_EML_IF(0,2,478,495,-1,534);
        _SFD_CV_INIT_EML_FOR(0,0,445,472,540);
...
static void sf_opaque_initialize_c6_sensor_block_together(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c6_sensor_block_together();
  initialize_c6_sensor_block_together();
}

static void sf_opaque_enable_c6_sensor_block_together(void *chartInstanceVar)
{
  enable_c6_sensor_block_together();
}

static void sf_opaque_disable_c6_sensor_block_together(void *chartInstanceVar)
{
  disable_c6_sensor_block_together();
}

static void sf_opaque_gateway_c6_sensor_block_together(void *chartInstanceVar)
{
  sf_c6_sensor_block_together();
}

static mxArray* sf_opaque_get_sim_state_c6_sensor_block_together(void
  *chartInstanceVar)
{
  mxArray *st = (mxArray *) get_sim_state_c6_sensor_block_together();
  return st;
}

static void sf_opaque_set_sim_state_c6_sensor_block_together(void
  *chartInstanceVar, const mxArray *st)
{
  set_sim_state_c6_sensor_block_together(sf_mex_dup(st));
}

static void sf_opaque_terminate_c6_sensor_block_together(void *chartInstanceVar)
{
  ...
  finalize_c6_sensor_block_together();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c6_sensor_block_together(SimStruct *S)
{
  int i;
  ...
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    ...
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"sensor_block_together",
        "sensor_block_together",6,10);
      sf_mark_chart_reusable_outputs(S,"sensor_block_together",
        "sensor_block_together",6,3);
    }

    sf_set_rtw_dwork_info(S,"sensor_block_together","sensor_block_together",6);
    ssSetHasSubFunctions(S,!(chartIsInlinable));
    ssSetOptions(S,ssGetOptions(S)|SS_OPTION_WORKS_WITH_CODE_REUSE);
  }

  ssSetChecksum0(S,(2857901879U));
  ssSetChecksum1(S,(2609042507U));
  ssSetChecksum2(S,(1566452960U));
  ssSetChecksum3(S,(2103802564U));
  ssSetmdlDerivatives(S, NULL);
  ssSetExplicitFCSSCtrl(S,1);
}

static void mdlRTW_c6_sensor_block_together(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c6_sensor_block_together(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c6_sensor_block_together;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c6_sensor_block_together;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c6_sensor_block_together;
  chartInstance.chartInfo.enableChart =
    sf_opaque_enable_c6_sensor_block_together;
  chartInstance.chartInfo.disableChart =
    sf_opaque_disable_c6_sensor_block_together;
  chartInstance.chartInfo.getSimState =
    sf_opaque_get_sim_state_c6_sensor_block_together;
  chartInstance.chartInfo.setSimState =
    sf_opaque_set_sim_state_c6_sensor_block_together;
  chartInstance.chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c6_sensor_block_together;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c6_sensor_block_together;
  chartInstance.chartInfo.mdlStart = mdlStart_c6_sensor_block_together;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c6_sensor_block_together;
...
void c6_sensor_block_together_method_dispatcher(SimStruct *S, int_T method, void
  *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c6_sensor_block_together(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c6_sensor_block_together(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c6_sensor_block_together(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c6_sensor_block_together_method_dispatcher.\n"
