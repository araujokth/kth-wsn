...
@title: README file for the Water Tank app on TDMA-CTP communication protocol
@date: 2013/01

DESCRIPTION
--------------------------------------------------
This application is for the Water Tank experiments, it uses TDMA-CTP as its routing layer protocol.
...
FOLDERS AND FILES
--------------------------------------------------
Before running this application, put the current folder in ~/local/src/tinyos-2.x/apps/ and make following modifications:

1) rename the folder EasyCollection to EasyCollection_Ring
   and rename the current folder EasyCollection_WT to EasyCollection;

2) rename these files:

 <PATH> tinyos-2.x-contrib/kth/tkn154-gts/tos/lib/mac/tkn154/
	TKN154ActiveMessageP.nc => TKN154ActiveMessageP_BREAKING.nc
	TKN154ActiveMessageP_NO_BREAKING.nc => TKN154ActiveMessageP.nc

 <PATH> tinyos-2.x-contrib/kth/tkn154-gts/tos/lib/mac/tkn154-sink/
	TKN154ActiveMessageP.nc => TKN154ActiveMessageP_RING.nc
	TKN154ActiveMessageP_TWO_PATH_WT.nc => TKN154ActiveMessageP.nc

 <PATH> tos/lib/net/ctp_tdma/
	CtpForwardingEngineP.nc => CtpForwardingEngineP_NO_WT.nc
	CtpForwardingEngineP_TWO_PATH_WT.nc => CtpForwardingEngineP.nc
	-- AND --
	CtpRoutingEngineP.nc => CtpRoutingEngineP_RING.nc
	CtpRoutingEngineP_TWO_PATH_WT.nc => CtpRoutingEngineP.nc
