...
implementation {
	...
	//OpenQueue
	async command OpenQueueEntry_t* OpenQueue.inQueueBySeed(uint8_t seedNumber) {
		uint8_t i;
		for (i=0;i<QUEUELENGTH;i++) {
			if (queue[i].l2_seedNumber == seedNumber)
				return &queue[i];
		}
		return NULL;
	}

	async command OpenQueueEntry_t* OpenQueue.inQueueToChild(uint8_t seedNumber) {
		uint8_t i;
		for (i=0;i<QUEUELENGTH;i++) {
			if ((queue[i].l2_seedNumber != 255) && (queue[i].l2_seedNumber != seedNumber ))
				return &queue[i];
		}
		return NULL;
	}
