...
static SFc2_sensor_block_pulseInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c2_sensor_block_pulse(void);
static void initialize_params_c2_sensor_block_pulse(void);
static void enable_c2_sensor_block_pulse(void);
static void disable_c2_sensor_block_pulse(void);
static void c2_update_debugger_state_c2_sensor_block_pulse(void);
static const mxArray *get_sim_state_c2_sensor_block_pulse(void);
static void set_sim_state_c2_sensor_block_pulse(const mxArray *c2_st);
static void finalize_c2_sensor_block_pulse(void);
static void sf_c2_sensor_block_pulse(void);
static void c2_c2_sensor_block_pulse(void);
...
static void initialize_c2_sensor_block_pulse(void)
{
  ...
  chartInstance.c2_is_active_c2_sensor_block_pulse = 0U;
}
...
static const mxArray *get_sim_state_c2_sensor_block_pulse(void)
{
  ...
  c2_g_u = chartInstance.c2_is_active_c2_sensor_block_pulse;
  c2_h_y = NULL;
...
static void set_sim_state_c2_sensor_block_pulse(const mxArray *c2_st)
{
  ...
  uint32_T c2_u5;
  uint32_T c2_f_y;
  const mxArray *c2_b_is_active_c2_sensor_block_pulse;
  uint8_T c2_u6;
  uint8_T c2_g_y;
  uint16_T *c2_f_delay;
  uint16_T *c2_pck_success;
  ...
  *c2_b_timestamp = c2_f_y;
  c2_b_is_active_c2_sensor_block_pulse = sf_mex_dup(sf_mex_getcell(c2_u, 6));
  sf_mex_import("is_active_c2_sensor_block_pulse", sf_mex_dup
                (c2_b_is_active_c2_sensor_block_pulse), &c2_u6, 1, 3, 0U, 0, 0U,
                0);
  c2_g_y = c2_u6;
  sf_mex_destroy(&c2_b_is_active_c2_sensor_block_pulse);
  chartInstance.c2_is_active_c2_sensor_block_pulse = c2_g_y;
  sf_mex_destroy(&c2_u);
  c2_update_debugger_state_c2_sensor_block_pulse();
...
const mxArray *sf_c2_sensor_block_pulse_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c2_sensor_block_pulse_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c2_sensor_block_pulse_get_autoinheritance_info(void)
{
...
static mxArray *sf_get_sim_state_info_c2_sensor_block_pulse(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  char *infoEncStr[] = {
    "100 S1x7'type','srcId','name','auxInfo'{{M[1],M[8],T\"delay\",},{M[1],M[12],T\"pck_success\",},{M[1],M[14],T\"pck_total\",},{M[1],M[11],T\"sensor_v\",},{M[1],M[13],T\"src_addr\",},{M[1],M[5],T\"timestamp\",},{M[8],M[0],T\"is_active_c2_sensor_block_pulse\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 7, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c2_sensor_block_pulse_get_check_sum(&mxChecksum);
...
static void sf_opaque_initialize_c2_sensor_block_pulse(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c2_sensor_block_pulse();
  initialize_c2_sensor_block_pulse();
}

static void sf_opaque_enable_c2_sensor_block_pulse(void *chartInstanceVar)
{
  enable_c2_sensor_block_pulse();
}

static void sf_opaque_disable_c2_sensor_block_pulse(void *chartInstanceVar)
{
  disable_c2_sensor_block_pulse();
}

static void sf_opaque_gateway_c2_sensor_block_pulse(void *chartInstanceVar)
{
  sf_c2_sensor_block_pulse();
}

static mxArray* sf_opaque_get_sim_state_c2_sensor_block_pulse(void
  *chartInstanceVar)
{
  mxArray *st = (mxArray *) get_sim_state_c2_sensor_block_pulse();
  return st;
}

static void sf_opaque_set_sim_state_c2_sensor_block_pulse(void *chartInstanceVar,
  const mxArray *st)
{
  set_sim_state_c2_sensor_block_pulse(sf_mex_dup(st));
}

static void sf_opaque_terminate_c2_sensor_block_pulse(void *chartInstanceVar)
{
  ...
  finalize_c2_sensor_block_pulse();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c2_sensor_block_pulse(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c2_sensor_block_pulse(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("sensor_block_pulse","sensor_block_pulse",2);
    ...
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("sensor_block_pulse",
      "sensor_block_pulse",2,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"sensor_block_pulse",
        "sensor_block_pulse",2,1);
      sf_mark_chart_reusable_outputs(S,"sensor_block_pulse","sensor_block_pulse",
        2,6);
    }

    sf_set_rtw_dwork_info(S,"sensor_block_pulse","sensor_block_pulse",2);
...
static void mdlRTW_c2_sensor_block_pulse(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "sensor_block_pulse", "sensor_block_pulse",2);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c2_sensor_block_pulse(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c2_sensor_block_pulse;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c2_sensor_block_pulse;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c2_sensor_block_pulse;
  chartInstance.chartInfo.enableChart = sf_opaque_enable_c2_sensor_block_pulse;
  chartInstance.chartInfo.disableChart = sf_opaque_disable_c2_sensor_block_pulse;
  chartInstance.chartInfo.getSimState =
    sf_opaque_get_sim_state_c2_sensor_block_pulse;
  chartInstance.chartInfo.setSimState =
    sf_opaque_set_sim_state_c2_sensor_block_pulse;
  chartInstance.chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c2_sensor_block_pulse;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c2_sensor_block_pulse;
  chartInstance.chartInfo.mdlStart = mdlStart_c2_sensor_block_pulse;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c2_sensor_block_pulse;
...
void c2_sensor_block_pulse_method_dispatcher(SimStruct *S, int_T method, void
  *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c2_sensor_block_pulse(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c2_sensor_block_pulse(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c2_sensor_block_pulse(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c2_sensor_block_pulse_method_dispatcher.\n"
