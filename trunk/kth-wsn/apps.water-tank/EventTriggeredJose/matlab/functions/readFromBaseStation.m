...
switch(WTSelf.controller)
    ...
    case WTSelf.SELFTRIGGERED_CONTROLLER
       if ref ==  WTSelf.INITIAL_REFERENCE
            WTSelf.logger.x0_initial(1,:,:)= double(y)/WTSelf.WT_CALIBRATION; 
       elseif ref == WTSelf.REFERENCE
            [WTSelf.logger.timeSchedule(n,:), WTSelf.logger.tau(n,:), ...
                ...
                integral_initial, u_initial,WTSelf.logger.x0_initial);
       elseif ref == WTSelf.END_REFERENCE
            % Save the logger
            waterTanksSelfTriggered('saveInformation');
            WTSelf.END_REFERENCE = 30;
       end
        
    case WTSelf.MODELING_CONTROLLER
        % constant values
WTModeling.u_5cm = [4.363];
 %  WTModeling.u_5cm = [8];
WTModeling.u_10cm = [6];
...
WTModeling.controller(:, :, 1) = [-0.9523   -0.0395   -0.0543];
        ...
        elseif WTModeling.state == WTModeling.STEP_RESPONSE
            ...
            for i=1:WTSelf.nWaterTanks
                state = [(double(y(i,:))/WTSelf.WT_CALIBRATION - ref) integral(i)];
        ...
        elseif WTModeling.state == WTModeling.INCR_RESPONSE
            ...
            if WTSelf.logger.nSamples >= ceil(WTModeling.STEADY_STATE_TIME(1,3)/WTSelf.tBI)
                ...
                WTModeling.parameters.gamma(:) = (double(y(:,2)))/(double(y(:,1)));
                % Go to the next state
...
WTSelf.logger.y(n,:,:) = y;
WTSelf.logger.x(n,:,:) = double(y)/WTSelf.WT_CALIBRATION;


WTSelf.logger.y_initial(n,:,:) = y_initial;
WTSelf.logger.u(n,:,:) = u;
WTSelf.logger.u_initial(n,:,:) = u_initial;
WTSelf.lofgger.integral(n,:) = integral;
