function [timeSchedule, tau, ttStart, lateMotes, slots ] = updateTimeslots(y, extraSensors, u, time, integral, y_initial,integral_initial, u_initial )
global WTSelf;
global COMM;
WTSelf.DEBUG = false;

        
end_integral = [-205 -87.4];
end_upper = [11.2 10.93];
...
for i=1:WTSelf.nWaterTanks
    ...
    if (WTSelf.timeSchedule( sensorIdx ) <= ttStart_next  + WTSelf.deltaCAP) && ~sum( WTSelf.lateMote == ( sensorIdx ) )
        x10=double(y(i,1))/WTSelf.WT_CALIBRATION - end_upper(i);
        x20=double(y(i,2))/WTSelf.WT_CALIBRATION - WTSelf.REFERENCE;
        x30=double(integral(i)-end_integral(i));
       %  x1´´ = x1´ - x1_final = x1_measured - x1_operation_point - x1_final

        WTSelf.logger.x10(WTSelf.logger.nSamples) = x10;
        WTSelf.logger.x20(WTSelf.logger.nSamples) = x20;
