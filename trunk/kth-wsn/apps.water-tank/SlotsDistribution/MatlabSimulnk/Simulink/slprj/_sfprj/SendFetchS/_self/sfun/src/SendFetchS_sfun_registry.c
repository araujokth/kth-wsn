...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_SendFetchS_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_SendFetchS_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_SendFetchS_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_SendFetchS_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_SendFetchS_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "SendFetchS") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_SendFetchS_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_SendFetchS_method_dispatcher(SimStruct *S, unsigned int
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"SendFetchS")) {
    return(sf_SendFetchS_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void SendFetchS_terminator(void);
...
extern void SendFetchS_initializer(void);
extern void SendFetchS_register_exported_symbols(SimStruct* S);
extern void SendFetchS_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  SendFetchS_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      SendFetchS_debug_initialize();
    }

    SendFetchS_initializer();
  }
