...
% if it is the first time the updatetimes is called
if WTSelf.initkkk==1
    WTSelf.timess=time;
    WTSelf.initkkk=0;
end
...
TAU_K = 10000 * ones(1, WTSelf.nWaterTanks + WTSelf.nSensors);          %set tau_k's to huge value so that we don't mess with calculating the min later on
        ...
        ttStart = time-WTSelf.timess;                             % time in symbols for the beacon broadcast
...
%         next_time=WTSelf.tBI; % Added for debugging
    ...
    % faster!
    nSensors=WTSelf.nSensors;
    
for kkk=1:WTSelf.nSensors
        node = 2* WTSelf.nWaterTanks + kkk;
        timefornode=WTSelf.timeSchedule(node) ;
        ...
        %     end
        %     end
        %% XXXXXXXXXXXXXXXXXXX low priority nodes to assign new times
        % ~sum( WTSelf.lateMote == node ) means that the mote is not late since
        % its not in the lateMote list
        %ttt=ttStart_next + WTSelf.deltaCAP
        %lateshit=~sum( WTSelf.lateMote == node )
        %lateshit2=WTSelf.lateMote
        if (WTSelf.timeSchedule(node) <= ttStart_next + WTSelf.deltaCAP) %&& ~sum( WTSelf.lateMote == node )  % control loops
            
            next_time = 0;%1 + (WTSelf.nWaterTanks + i - 2 )*rand(1);
            next_time = 0;%8*WTSelf.tBI;
          %  node
          %  allocationoftime=WTSelf.requesttimes{kkk}(1,WTSelf.pCurrentPkt(1,i))
            WTSelf.timeSchedule(node) =  WTSelf.requesttimes{i}(1,WTSelf.pCurrentPkt(1,kkk)); %WTSelf.timeSchedule(node) + real(next_time);    % time_schedule stores the value of tk+1
            TAU_K(WTSelf.nWaterTanks +kkk) = real(next_time);                               % store all the periods
            
            if WTSelf.DEBUG
                fprintf(' => Node %d was scheduled! => next time: %f => new scheduled time: %f \n', node, TAU_K(WTSelf.nWaterTanks +kkk),  WTSelf.timeSchedule(node));
            end
        end
 end
...
for k=1:WTSelf.nWaterTanks*2
    ...
    if WTSelf.timeSchedule(k) <= ttStart_next + WTSelf.tBI + WTSelf.deltaCAP %&& WTSelf.timeSchedule(i)>=ttStart_next+WTSelf.deltaCAP
        if WTSelf.DEBUG  
            fprintf(' => Node %u should get a slot in the next beacon interval!\n', k); end
...
for k=WTSelf.nWaterTanks*2+1:WTSelf.nTotal
    if WTSelf.DEBUG 
    fprintf('Node %d: %f', k, WTSelf.timeSchedule(k));
    if WTSelf.timeSchedule(k) <= ttStart_next
        fprintf(' (*)');
    end
    fprintf('\n');
    end
    SDvalue=WTSelf.SD;
    if WTSelf.timeSchedule(k) <= ttStart_next + WTSelf.SD %&& WTSelf.timeSchedule(i)>=ttStart_next+WTSelf.deltaCAP
    ...
    if (node > WTSelf.nWaterTanks * 2)
        if slot_num <= WTSelf.cfpSlots
            ...
            WTSelf.timeSchedule(node)   = ttStart_next + WTSelf.deltaCAP + (slot_num-1) * WTSelf.slotLength * WTSelf.SYMBOL;
             ...
             %% XXXXXXXXXXXXXXX new line for low priority nodes.
            % triggtimes will save the triggering times for post processing
            %node
            %WTSelf.nWaterTanks * 2
           % if node==9
          %  node
           % NEXTTIMETOIGGER=WTSelf.timeSchedule(node)
           % pointervalue=WTSelf.pCurrentPkt(1,node-WTSelf.nWaterTanks * 2)
           % requestimeslots=WTSelf.requesttimes{node-WTSelf.nWaterTanks * 2}(1,WTSelf.pCurrentPkt(1,node-WTSelf.nWaterTanks * 2))
           % requesttimeslotsforall=WTSelf.requesttimes{node-WTSelf.nWaterTanks * 2};
           % end
            WTSelf.requesttimes{node-WTSelf.nWaterTanks * 2}(1,WTSelf.pCurrentPkt(1,node-WTSelf.nWaterTanks * 2));
            WTSelf.triggtimes{node-WTSelf.nWaterTanks * 2}(1,WTSelf.pCurrentPkt(1,node-WTSelf.nWaterTanks*2))=WTSelf.timeSchedule(node);
            WTSelf.pCurrentPkt(1,node-WTSelf.nWaterTanks*2)=WTSelf.pCurrentPkt(1,node-WTSelf.nWaterTanks*2)+1 ;% increase pointer
...
%% XXXXXXXXXXXXXXX do some stats calculations for the low priority nodes with respect to
%% the delay

for kkk=1:WTSelf.nSensors % only for low priority nodes 
  WTSelf.triggtimes{kkk};
  WTSelf.requesttimes{kkk};
WTSelf.Nsamplesrequested(kkk,1)=length(WTSelf.requesttimes{kkk});
WTSelf.Nsamplestriggered(kkk,1)=length(WTSelf.triggtimes{kkk});
end
