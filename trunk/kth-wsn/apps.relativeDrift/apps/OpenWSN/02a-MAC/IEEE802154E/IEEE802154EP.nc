...
implementation {
	...
	uint32_t A_interval;
	uint32_t B_interval;
	uint32_t numSlotsToResynchronize;
	uint32_t counterSlotsToResynchronize;
	uint32_t Tr_resynchTime;
	bool drift;
	double a_drift;
	double a_currentDrift;
	double b_offset;
	double b_currentOffset;
	double Tr1_resynchTime;
	double Tr2_resynchTime;
	double tmp1;
	double tmp2;
	uint32_t numADVtoResynchronize;
	uint32_t counterNumADVtoResynchronize;
...
#include "IEEE802154_common.c"
	...
	async event void SlotAlarm.fired() {
		...
		atomic {
			if (change) {
				...
				numSlotsInSynchStatus=0;
				numPrepareReceiveFailed=0;
				numSlotsToResynchronize = 0;
				counterSlotsToResynchronize = 0;
				drift = FALSE;
			}
		}
			...
			if (call IDManager.isMyAddress(&(received_ieee154_header.dest)) && received_ieee154_header.ackRequested) {
				...
				if (received_ieee154_header.frameType==IEEE154_TYPE_CMD &&
					...
					if (numADV==1) {
						...
						printf("%lu %lu\n", timestampX1, timestampY1);
						printfflush();
					}
					if (numADV==240) {
						timestampX2 = ((IEEE802154E_ADV_t*)(msg->payload))->timestampInformation;
						timestampY2 = call SlotAlarm.getNow();
						a_drift = ((double)(timestampY2-timestampY1))/((double)(timestampX2-timestampX1));
						b_offset = ((double)timestampY2) - a_drift*((double)timestampX2);
						printf("%lu %lu ", timestampX2, timestampY2);
						printfFloat((float)a_drift);
						printfFloat((float)b_offset);
						printf("\n");
						printfflush();
						timestampX1 = timestampX2;
						timestampY1 = timestampY2;
						//						drift = TRUE;
						//						call CriticalTime.syncDone();
						call Leds.led0Toggle();
					} else if (numADV==480 || numADV == counterNumADVtoResynchronize) {
						timestampX2 = ((IEEE802154E_ADV_t*)(msg->payload))->timestampInformation;
						timestampY2 = call SlotAlarm.getNow();
						a_currentDrift = ((double)(timestampY2-timestampY1))/((double)(timestampX2-timestampX1));
						b_currentOffset = ((double)timestampY2) - a_currentDrift*((double)timestampX2);
						tmp1 = (((double)GAMMA)*((double)(a_drift - a_currentDrift)));
						tmp2 = (((double)GAMMA)*((double)(b_offset - b_currentOffset)));
						Tr1_resynchTime = ((((double)D_THRESHOLD)-(tmp2))/(tmp1));
						Tr2_resynchTime = ((-((double)D_THRESHOLD)-(tmp2))/(tmp1));
						if (Tr1_resynchTime > Tr2_resynchTime)
						{
							Tr_resynchTime = (uint32_t)Tr1_resynchTime;
						} else {
							Tr_resynchTime = (uint32_t)Tr2_resynchTime;
						}
						if (Tr_resynchTime>3932105) Tr_resynchTime=3932105;
						numADVtoResynchronize = (Tr_resynchTime)/(8191);
						counterNumADVtoResynchronize = numADV + ((uint32_t)numADVtoResynchronize);
						printf("%lu %lu ", timestampX2, timestampY2);
						printfFloat((float)a_currentDrift);
						printfFloat((float)b_currentOffset);
						printfFloat((float)tmp1);
						printfFloat((float)tmp2);
						printfFloat((float)Tr1_resynchTime);
						printfFloat((float)Tr2_resynchTime);
						printf(" %lu",Tr_resynchTime);
						printf("\n");
						printfflush();
						timestampX1 = timestampX2;
						timestampY1 = timestampY2;
						a_drift = tmp1 + a_currentDrift;
						b_offset = tmp2 + b_currentOffset;
					}
				...
				else if (received_ieee154_header.frameType==IEEE154_TYPE_DATA)
				{
					atomic
					{
						if (numADV >=240 )
						{
							...
							numReceiveNothing=0;
							if (drift) {
								counterSlotsToResynchronize = counterSlotsToResynchronize-1;
								if (counterSlotsToResynchronize==0) {
									counterSlotsToResynchronize=numSlotsToResynchronize;
									resynchronize(FRAME_BASED_RESYNC,
											&received_ieee154_header.src,
											msg->l1_rxTimestamp,
											(int16_t)((int32_t)(msg->l1_rxTimestamp)-(int32_t)radio_delay)-(int32_t)TsTxOffset);
	...
	void printfFloat(float toBePrinted) {
		uint32_t fi, f0, f1, f2, f3, f4, f5, f6, f7, f8;
		char c;
		float f = toBePrinted;
		...
		f8 = f * 1000000000;
		f8 %= 10;
		printf("%c%ld.%d%d%d%d%d%d%d%d%d", c, fi, (uint8_t) f0, (uint8_t) f1, (uint8_t) f2, (uint8_t) f3, (uint8_t) f4, (uint8_t) f5, (uint8_t) f6, (uint8_t) f7, (uint8_t) f8);
	}
	uint32_t round(float number) {
