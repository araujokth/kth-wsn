			...
			if (call IDManager.isMyAddress(&(received_ieee154_header.dest)) && received_ieee154_header.ackRequested) {
				...
				if (received_ieee154_header.frameType==IEEE154_TYPE_CMD &&
					...
					if (numADV==240) {
						...
						// Compute Resynchronization Time Tr
						A_interval = timestampX2 - timestampX1;
						B_interval = timestampY2 - timestampY1;
						tmp1 = (uint64_t)(D_THRESHOLD*A_interval);
						tmp2 = (uint64_t)(A_interval-(2*DELTA_UNCERTAINTY));
						tmp3 = (uint64_t)(2*DELTA_UNCERTAINTY*(A_interval+B_interval));
						Tr_resynchTime = (tmp1*tmp2)/tmp3;
						numSlotsToResynchronize = ((uint32_t)(Tr_resynchTime))/((uint32_t)(SLOT_TIME));
