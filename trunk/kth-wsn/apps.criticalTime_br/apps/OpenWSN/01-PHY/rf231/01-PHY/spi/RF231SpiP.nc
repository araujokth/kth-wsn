module RF231SpiP @safe() {
  ...
  uses {
    ...
    interface Leds;
    interface BusyWait<TMicro, uint16_t> as spiWait;
  }
}
...
implementation {
  ...
  enum {
    RESOURCE_COUNT = uniqueCount( "RF231Spi.Resource" ),
    NO_HOLDER = 0xFF,
  };
...
/** Reading packet or Writing packet? */
enum {
   WAS_READING,
   WAS_WRITING,
};

  /** Address to read/write on the RF231, also maintains caller's client id */
  ...
  /** Used in SpiPacket.sendDone to know which function to call */
  uint8_t packetOperation;
  ...
  ***************** SpiPacket Events ****************/
  async event void SpiPacket.sendDone( uint8_t* tx_buf, uint8_t* rx_buf, 
    ...
    if ( packetOperation == WAS_WRITING ) {
      signal ChipSpiResource.loadPacketDone(error);
    } else if (packetOperation == WAS_READING ){
...
async command void ChipSpiResource.changeState( uint8_t newState ) {
    atomic {
      if(call WorkingState.isIdle()) {
        return;
      }
    }
    call SpiByte.write( AT_WRITE_REG(RG_TRX_STATE) );
    call SpiByte.write( newState );
  }

async command uint8_t ChipSpiResource.readRegister(uint8_t registerAddress ) {
atomic {
      if(call WorkingState.isIdle()) {
        return 0;
      }
    }
    call SpiByte.write( AT_READ_REG(registerAddress) );
    return call SpiByte.write( 0x00 );//doesn't really matter: could be sending zeros
  }

async command void ChipSpiResource.setChannel(uint8_t newChannel ) {
    atomic {
      if(call WorkingState.isIdle()) {
        return;
      }
    }
    call SpiByte.write( AT_WRITE_REG(RG_PHY_CC_CCA) );
    call SpiByte.write( 0x20 | newChannel );//to understand 0x20, check out p.122 of datasheet

    signal ChipSpiResource.doneSettingChannel();
  }


async command void ChipSpiResource.setRegister(uint8_t regAddr, uint8_t newValue ) {
    atomic {
      if(call WorkingState.isIdle()) {
        return;
      }
    }
    call SpiByte.write( AT_WRITE_REG(regAddr) );
    call SpiByte.write( newValue );
  }

async command void ChipSpiResource.loadPacket(uint8_t* data, uint8_t len) {
uint8_t status = 0;
    atomic {
      if(call WorkingState.isIdle()) {
        return;
    ...
    atomic packetOperation = WAS_WRITING;

    status = call SpiByte.write( 0x60 );//this should be the command to tell the radio that the packet is following
    call SpiByte.write(len); //write the length of the frame
    call SpiPacket.send( data, NULL, len );//write the packet

    //return status;//should call something?// no because loadpackeddone is calling it
}

async command void ChipSpiResource.readPacket(uint8_t* data){
uint8_t len = 0;
    atomic {
      if(call WorkingState.isIdle()) {
        return;
...
call SpiByte.write(0x60);
len = call SpiByte.write(0);
data = malloc(len);
*data++ = len;
    for ( ; len; len-- ) {
      *data++ = call SpiByte.write( 0 );
